...
sitegen/          the generator (pure python; numpy for pose curves, everything else stdlib)
  nav.py          THE page set — one entry per tab; NAV[0] is the site home
  ...
  report_page.py  report.html   <- data/report.json
  vio_filter_page.py  vio_filter.html <- data/vio_review/ + data/calib/
  yaw_review_page.py  yaw_review.html <- data/yaw_review/
  build.py        rebuild every page; --check diffs against what's committed
data/             committed input snapshots (~16 MB; the clips live in the site's media/, not here)
...
index.html        root redirect -> the first page in NAV
```

## Commands

```bash
python3 -m sitegen.build            # rebuild every page in place
...
uv run --no-project --with pytest --with numpy python -m pytest tests/ -q   # 52 tests, ~1s
...
- **Sections, not markup.** A builder returns a list of section dicts (`{"h"}`, `{"text"}`,
...
- **Adding/removing a tab** is `sitegen/nav.py` plus a builder module. Every page renders the same
  `NAV`, and `build.py` refuses to run if `NAV` lists a page with no builder.
